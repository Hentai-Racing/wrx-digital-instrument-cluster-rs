...
pub async fn run_vcan_generator(socket: &mut CanSocket, running: Arc<AtomicBool>) {
    while running.load(Ordering::SeqCst) {
        ...
        let xxxmsg209_frame = wrx_2018::XxxMsg209::new(
            ...
            .expect("Failed to create frame");
        if let Some(frame) = Frame::new(xxxmsg209_frame.id(), xxxmsg209_frame.data()) {
            socket.write_frame(frame).unwrap().await.unwrap();
        }
        ...
        let xxxmsg340_frame = wrx_2018::XxxMsg340::new(any_door_open)
            .expect("Failed to create frame");
        if let Some(frame) = Frame::new(xxxmsg340_frame.id(), xxxmsg340_frame.data()) {
            socket.write_frame(frame).unwrap().await.unwrap();
        }
        ...
        let xxxmsg640_frame = wrx_2018::XxxMsg640::new(
            ...
            .expect("Failed to create frame");
        if let Some(frame) = Frame::new(xxxmsg640_frame.id(), xxxmsg640_frame.data()) {
            socket.write_frame(frame).unwrap().await.unwrap();
        }
        ...
        let srsstatus_frame = wrx_2018::SrsStatus::new(srs_system_warning_light_enabled)
            .expect("Failed to create frame");
        if let Some(frame) = Frame::new(srsstatus_frame.id(), srsstatus_frame.data()) {
            socket.write_frame(frame).unwrap().await.unwrap();
        }
        ...
        let xxxmsg884_frame = wrx_2018::XxxMsg884::new(
            ...
            .expect("Failed to create frame");
        if let Some(frame) = Frame::new(xxxmsg884_frame.id(), xxxmsg884_frame.data()) {
            socket.write_frame(frame).unwrap().await.unwrap();
        }
        ...
        let xxxmsg885_frame = wrx_2018::XxxMsg885::new(
            ...
            .expect("Failed to create frame");
        if let Some(frame) = Frame::new(xxxmsg885_frame.id(), xxxmsg885_frame.data()) {
            socket.write_frame(frame).unwrap().await.unwrap();
        }
