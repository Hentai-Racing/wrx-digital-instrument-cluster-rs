...
pub fn run_bevy_app_with_slint(
    ...
    crossbeam::channel::Receiver<wgpu::Texture>,
    crossbeam::channel::Sender<ControlMessage>,
) {
    let (control_message_sender, control_message_receiver) =
        crossbeam::channel::bounded::<ControlMessage>(2);
    let (bevy_front_buffer_sender, bevy_front_buffer_receiver) =
        crossbeam::channel::bounded::<wgpu::Texture>(2);
    ...
    let _bevy_thread = std::thread::spawn(move || {
        let runner = move |mut app: bevy::app::App| {
            ...
            loop {
                let mut next_back_buffer = match control_message_receiver.recv() {
    ...
    control_message_sender
        .send(ControlMessage::ReleaseFrontBufferTexture {
            texture: back_buffer,
        })
        .unwrap();
    control_message_sender
...
struct FrontBufferReturnSender(crossbeam::channel::Sender<wgpu::Texture>);
/// Plugin for Render world part of work
struct SlintRenderToTexturePlugin(crossbeam::channel::Sender<wgpu::Texture>);
impl Plugin for SlintRenderToTexturePlugin {
    fn build(&self, app: &mut App) {
        let render_app = app.sub_app_mut(RenderApp);

        let mut graph = render_app.world_mut().resource_mut::<RenderGraph>();
        graph.add_node(SlintSwapChain, SlintSwapChainDriver::default());
...
impl render_graph::Node for SlintSwapChainDriver {
    fn run(
        ...
        if let Some(bb) = &back_buffer.0 {
            front_buffer_sender.0.send(bb.clone()).ok();
        } else {
            bevy::log::warn!("SlintSwapChain: BackBuffer is None, skipping frame");
        }
