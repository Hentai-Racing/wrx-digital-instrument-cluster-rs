/// Modified from original Slint example https://github.com/slint-ui/slint/tree/master/examples/maps
///
...
impl World {
    fn new() -> Self {
        World {
            ...
            osm_url: "https://tile.openstreetmap.org".into(),
...
impl MapState {
    ...
    fn refresh_model(&self) {
        if let Some(ui) = self.ui.upgrade() {
            ...
            let map_properties = ui.global::<NavMapProperties>();

            map_properties.set_tiles(slint::ModelRc::new(vec));
        }
    }
    ...
    fn set_viewport_size(&self) {
        if let Some(ui) = self.ui.upgrade() {
            ...
            map_properties.set_zoom(zoom as _);
            let world_size = (TILE_SIZE * (1 << zoom)) as f32;
            map_properties.invoke_set_viewport(
...
impl NavMap {
    pub fn new(ui: Weak<App>) -> Self {
        let this = Self {
            state: Rc::new(MapState::new(ui.clone())),
            ui,
        };
        ...
        if let Some(ui) = this.ui.upgrade() {
            ...
            let state_weak = Rc::downgrade(&this.state);
            let ui_weak = ui.as_weak();
            map_properties.on_flicked(move |ox, oy| {
                if let Some(ui) = ui_weak.upgrade() {
                    ...
                    world.visible_width = map_properties.get_visible_width() as f64;
                    world.visible_height = map_properties.get_visible_height() as f64;
                    world.reset_view();
                    drop(world);
                    state.do_poll();
                }
            ...
            let state_weak = Rc::downgrade(&this.state);
            let ui_weak = ui.as_weak();
            map_properties.on_zoom_changed(move |zoom| {
                if let Some(ui) = ui_weak.upgrade() {
                    let map_properties = ui.global::<NavMapProperties>();
                    let state = state_weak.upgrade().unwrap();
                    let mut world = state.world.borrow_mut();
            ...
            let state_weak = Rc::downgrade(&this.state);
            let ui_weak = ui.as_weak();
            map_properties.on_zoom_in(move |ox, oy| {
                if let Some(ui) = ui_weak.upgrade() {
                    let map_properties = ui.global::<NavMapProperties>();
                    let state = state_weak.upgrade().unwrap();
                    let mut world = state.world.borrow_mut();
            ...
            let state_weak = Rc::downgrade(&this.state);
            let ui_weak = ui.as_weak();
            map_properties.on_zoom_out(move |ox, oy| {
                if let Some(ui) = ui_weak.upgrade() {
                    let map_properties = ui.global::<NavMapProperties>();
                    let state = state_weak.upgrade().unwrap();
                    let mut world = state.world.borrow_mut();
